...
export class UserNftComponent implements OnInit {
  ...
  constructor(
    private userNft: IssueNftService,
  ...
  ngOnInit(): void {
    ...
    this.userNft.getuserbyuserName(this.userName).subscribe(
      (res: any) => {
        console.log(res.nfts);
        this.length = res.nfts.length;
        this.newNfts = res.nfts;
        this.newFilterNftsData = res.nfts;
        console.log(res);
        console.log(length);
    ...
    if (this.newNfts.length === 0 || this.filteredData === '') {
      ...
      return this.newNfts.filter((filter) => {
        ...
        return (
          ...
          filter.nftId.toLowerCase().includes(filterterm.toLocaleLowerCase()) ||
          filter.fName.toLowerCase().includes(filterterm.toLocaleLowerCase()) ||
          filter.lName.toLowerCase().includes(filterterm.toLocaleLowerCase()) ||
          filter.masteryLevel
            .toLowerCase()
            .includes(filterterm.toLocaleLowerCase()) ||
          filter.nftStatus
          ...
          filter.practice
            .toLowerCase()
            .includes(filterterm.toLocaleLowerCase()) ||
          filter.circle.toLowerCase().includes(filterterm.toLocaleLowerCase())
        );
      });
    }
  }
