...
export class HomePopupComponent implements OnInit {
  ...
  constructor(
    ...
    this.empDataService.getempData().subscribe((res) => {
      this.empData = res;
      ...
      this.loginService.login(this.adminLogin.value).subscribe(
        (res: any) => {
          ...
          if (
            ...
            this.isSnackClose = 2;
          } else if (res['message'] === 'Successfully LoggedIn') {
            this.loginService.currentUser =
              this.adminLogin.get('userName').value;
        ...
        (res: any) => {
          ...
          if (
            ...
            if (
              ...
              sessionStorage.setItem(
                'loggedInUser',
                this.userLogin.get('userName').value
              );
              this.loginService.isLoggedIn = true;
              ...
              res['message'] === 'Password Expired, Please Update The Password'
            ) {
              this.userLogin.reset();
              this.userLogin.get('userName').clearValidators();
              this.userLogin.get('userName').updateValueAndValidity();
      ...
      this.loginService.login(this.externalLogin.value).subscribe(
        (res: any) => {
          ...
          if (res['role'] === 'Admin' || res['role'] === 'user') {
            ...
            this.matSnackBarCloseAction();
            this.isSnackClose = 8;
          } else if (res['role'] === 'External' || res['role'] === 'external' || res['role'] === 'others' || res['role'] === 'Others') {
    ...
    if (this.updatePassword.valid) {
      this.loginService.update(this.updatePassword.value).subscribe(
        (res: any) => {
          ...
          } else if (res['message'] == 'password successfully updated') {
            this.isSnackBarOpen = true;
            this.isSnackBarDesp = 'Password successfully updated';
            this.matSnackBarCloseAction();
