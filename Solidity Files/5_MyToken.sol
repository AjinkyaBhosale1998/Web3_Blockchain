...
constructor () {
    balanceOf[msg.sender] = totalSupply;  // sending to the total balance
    emit Transfer(address(0), msg.sender, totalSupply);
}

function transfer(address _to, uint256 _value) public returns (bool success) {
    require(balanceOf[msg.sender] >= _value, "Not enough tokens to transfer.");
    //require is make sure the condition is fulfilled
    balanceOf[msg.sender] -= _value;
    balanceOf[_to] += _value;

    emit Transfer(msg.sender, _to, _value);  // to fire event
    return true;
}
